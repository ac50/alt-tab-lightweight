...
stored closure. It supports the **wrap pattern** — a control's existing action is read, captured as
`original`, and re-wrapped so an intercepting check can run first and then optionally fall through to
`original?(control)`.
...
- **testWrapPatternSkipsPreviousClosureOnEarlyReturn** — a wrapper that returns early does NOT fire the original, so the value is never written to `Preferences`.
